...
const API_URL = "https://api.popcat.xyz/jokeoverhead";
...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    const url = `${API_URL}?image=${encodeURIComponent(imageUrl.split("?")[0])}`;

    const buffer = await fetchImage(url);
    if (!buffer) return message.reply({ embeds: [EmbedUtils.error("Failed to generate image. Try again!")] });

    const attachment = new AttachmentBuilder(buffer, { name: "jokeoverhead.png" });
    const embed = EmbedUtils.embed()
      .setTitle("Joke Overhead")
